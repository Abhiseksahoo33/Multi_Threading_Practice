...
// 0. try_lock() Tries to lock the mutex not locking it. Returns immediately. On successful lock acquisition returns true otherwise returns false.

// 1. If try lock() is not able to lock mutex, then it doesn't get blocked that's why it is called non-blocking.

// 2. If try lock is called again by the same thread which owns the mutex, the behavior is undefined. (double try lock())

// It is a deadlock situation with undefined behavior. (if you want to be able to lock the same mutex by same thread

// more than one time to go for recursive_mutex)
...
// 7. std :: recursive mutex: :try_lock
// 8. std: : shared timed_mutex :: try_lock
